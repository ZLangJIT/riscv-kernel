function check_patch() {
    ...
    if ! wget -q https://github.com/ZLangJIT/riscv-kernel/releases/download/patches-$1/$1.patch -O $1.patch.latest
        then
...
LIBMEDIA_GRADLE_VERSION_CODE=$(($KERNEL_VERSION_CODE+$BUILDROOT_VERSION_CODE+$RVVM_VERSION_CODE+$VIRGL_VERSION_CODE+$LIBMEDIA_VERSION_CODE))

LIBMEDIA_GRADLE_VERSION_STRING="Linux Kernel 6.11 (kernel v$KERNEL_VERSION_MAJOR.$KERNEL_VERSION_MINOR)(buildroot+rootfs v$BUILDROOT_VERSION_MAJOR.$BUILDROOT_VERSION_MINOR)(rvvm v$RVVM_VERSION_MAJOR.$RVVM_VERSION_MINOR)(virglrenderer v$VIRGL_VERSION_MAJOR.$VIRGL_VERSION_MINOR)(libmedia v$LIBMEDIA_VERSION_MAJOR.$LIBMEDIA_VERSION_MINOR)"
